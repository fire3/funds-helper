...
| `sources` | 联接基金反查：`toFeederTarget`（联接基金 → `ETFCODE`，脏代码按「没查到」处理）、`fetchFeederTargets`（三分类、并发上限、进度回调、空列表不打上游）；东财：`parseEtfSpotPage`（字段映射、`f402` 语义、脏行、`data:null`、`total` 护栏）、`parseEtfQuoteResponse` + `fetchEtfSpotBySecids`（分片 100 只/请求、去重、非场内代码过滤、覆盖率护栏、真实响应 fixture）；新浪：`parseSinaEtfListPage`（单位换算 股→手 / 万元→元、振幅推导、脏行）、`parseSinaEtfCount`（`"1676"` 字符串、上限护栏）；`parseEtfProfilePage`（标志位、`null` 数值）+ `fetchEtfProfiles` 的截断护栏；`parseFundProfile`（`--` 归一化、`Datas:null`）|
| `db` | 迁移建出 4 张 `etf_*` 表（含 `etf_spot_daily.source`、`etf_feeder_fund` 的 `feeder_code` 主键约束）|
| `server` | 联接基金：未反查时不给用户请求打上游、全量落库后按 ETF join（A/C/I/Y 全在）、增量只查未落库的候选、上游为空的候选下次重查、单只失败不影响整轮且下次重试、候选池整体故障时 503 且不清空映射、全量结果塌方式缩水时不清扫、全量无失败时清掉已清盘的行、定时任务的 6 天门槛；偏好多东财时**把目录代码池交给批量报价**、偏好新浪时完全不碰东财、目录缺失时退回 `clist`、东财失败降级新浪、`PUT /config` 落库 + 立刻重抓（切回东财折溢价恢复）、非法渠道 400、`dataSource` 从数据库多数派反推、缺折溢价时 `premiumRate=null` 且 `unknown === total`、join 与分类、统计与覆盖率、`data_date`、幂等刷新、详情（接口 C + 通用区块）、400/404 |
| `web` | 分类/交易所维内 OR、维度间 AND、折溢价方向、规模/成交额档位、场外联接（存在性筛选）、URL 双向同步、无折溢价渠道下隐藏折溢价视图 |
| 一致性 | `ETF_CATEGORIES`（core ↔ shared）逐字一致 + 顺序一致；`ETF_SPOT_SOURCES` 与 shared 渠道枚举一一对应；注册表 4 个工具 |
| 端到端 | `pnpm smoke` 增加 ETF 检查（清单、refresh、dataset 数量与分类、渠道与缺失字段、折溢价按渠道分流断言、详情、场外联接字段契约）；`SMOKE_FEEDERS=1 pnpm smoke` 额外跑一次**真实全量反查**（约 4 分钟，实测 2317 只 / 0 失败 / 1020 只 ETF 覆盖）|
...
- **场外联接基金的口径边界**（§11.7）：`0 只` 同时表示「本来就没有」与「还没反查到」，
  界面文案因此写「未查到场外联接基金」；退市的联接基金只能靠手动全量重建清理
  （增量只做新增）；反查是**每周 + 启动补跑**的独立慢链路，最新一天内可能落后于行情。

---

## 11. 场外联接基金反查（ETF → 场外）

> 上游接口的逆向细节（字段、实测数字、限流）见
> [`etf-data-sources.md`](./etf-data-sources.md) §8；本节只写**领域口径与工程方案**。

### 11.1 要回答的问题

「这只 ETF 我没法场内买（没账户 / 想定投），对应的场外基金是哪些？」

方向很重要：QDII 工具里的接口 I 是**场外 → 场内**（联接基金的详情页显示它持有的 ETF，见
`packages/sources/src/eastmoney/holdings.ts`）。这里要的是**场内 → 场外**，
数据源相同（接口 I 的 `ETFCODE` 字段），但**查询方向相反**：
接口 I 只能按基金代码查，所以反查必须**先枚举全部候选联接基金**。

### 11.2 为什么不用「按跟踪指数 join」

最直觉的做法是「ETF 的跟踪指数 = 联接基金的跟踪指数」，但实测两个口径**对不上**：

| 基金 | 字段 | 值 |
|---|---|---|
| `159941` 纳指ETF广发 | 接口 B `INDEX_CODE` | `NDX` |
| `270042` 广发纳指100ETF联接A | 接口 C `INDEXCODE` | `NDX100` |

上游各自维护一套指数代码（`051` 段与 `NDX` 段），做映射等于再维护一张表。
接口 I 直接给**确切的目标 ETF 代码**，是权威口径 —— 因此本方案不做指数推断。

### 11.3 两段式采集

```
① 候选池（1 请求）
   接口 E：fundcode_search.js → 全市场 27905 只基金
   取名称含「联接」的 2317 只（core.isFeederFundName 判定）

② 反查（每只 1 请求，全量 2317）
   接口 I：FundMNInverstPosition?FCODE=<联接代码>
   → Datas.ETFCODE / Datas.ETFSHORTNAME / 顶层 Expansion（报告期）
```

实测全量结果（应用内正式链路）：**2304/2317 拿到 `ETFCODE`（0 失败）→ 1020 只 ETF，
全部命中接口 B 的目录**，即 **60.9%** 的 ETF 有场外联接基金
（其余 654 只是债券/商品/部分跨境主题，本来就没有）。

候选池**不能**用「名称含 ETF 联接」来筛：实测 2317 只里有 50 只不含「ETF」
（如 `000942 广发信息技术联接A` → `159939 信息技术ETF广发`），它们的 ETF 也在目录里。
详见 `core/etf/feeder.ts` 的说明。

**增量刷新**：联接关系变化极慢，因此每周只反查
「候选池里尚未落库的代码」+「上次返回为空的代码」（实测 13 只，多为新成立未建仓）。
首次全量扫描后，稳态每周只需**几十个请求**；断档时可手动触发全量重建。

### 11.4 请求预算

全量 2317 请求走**专用的 HTTP 通道**（并发 6 / 同 host 最小间隔 100ms）≈ **4 分钟**
（实测 **232.7 秒**）；共用的 `ctx.http`（并发 2 / 300ms）会让同样的量拖到 12 分钟。
这是本项目唯一一处偏离默认节流的链路，理由是它**一次性、可离线、不在用户请求路径上**，
且实测该接口在 20 并发下 3.7 秒跑完 2319 个请求、0 限流 —— 10 req/s 已留了 50 倍余量。
增量刷新只有几十个请求，走同一条通道也只是几秒钟，因此不按批量大小分流。

### 11.5 落库（`0009-etf-feeder-fund.ts`）

```sql
etf_feeder_fund(feeder_code PRIMARY KEY, etf_code, feeder_name, report_date, captured_at)
CREATE INDEX idx_etf_feeder_fund_etf ON etf_feeder_fund(etf_code)
```

- 主键取 **`feeder_code`**：「联接基金 → 目标 ETF」是唯一方向（一只联接基金只持有一只 ETF），
  而一只 ETF 有多个份额（A/C/E/I/F）——以联接基金为主键既表达了这对一对多，
  又让「联接基金换了目标 ETF」在 upsert 时自动收敛，不需要额外清理；
- 索引反过来建在 `etf_code` 上：读取路径是「按 ETF 取全部份额」（数据集 join）；
- `report_date` = 接口 I 的 `Expansion`（持仓报告期，如 `2026-06-30`）——
  它**晚于**建仓，因此新成立的联接基金可能暂时为空；
- 上次成功扫描的时刻存 `app_setting`（键 `etf.feederScannedAt`），
  用来判断「要不要跑增量/要不要全量」；
- **落库护栏**：全量走「事务内整表替换」（这样才能清掉已清盘的联接基金），
  但只在 `失败数 = 0` 且 `本轮映射数 ≥ 库里已有行数 × 0.5` 时才替换 ——
  否则退化为增量写入并告警。「上游抽风只回一小部分」与「联接基金真的批量清盘」
  在数据上无法区分，宁可保留旧数据。用相对比例而不是绝对条数，是因为绝对下限
  在首次建库（库是空的）场景下会把正常路径也挡掉。

### 11.6 接口与前端

| 位置 | 变化 |
|---|---|
| 数据集 | `EtfRecord.feederFunds: { code, name }[]`（按代码升序，空数组 = 没有联接基金）；响应新增 `feeder: { updatedAt, etfCount, fundCount }` |
| 列表 | 新增「场外联接」列，显示份额数（`--` = 无）；空列整体隐藏（与折溢价/上市日同规则） |
| 筛选 | 新增「场外联接」chip：`不限 / 有联接基金`（`hasFeeder`） |
| 抽屉 | 新增「场外联接基金」区块：逐个列出代码 + 简称，外链到天天基金对应页面 |
| 刷新 | `POST /api/tools/etf/feeders/refresh`（手动触发；全量重建用 `?full=1`） |
| 定时 | 新任务 `etf.feeders`，每周一 03:00（`0 3 * * 1`），启动时按「距上次成功 ≥7 天」补跑 |

### 11.7 口径边界（必须显式说明，否则会被误读）

- **`0 只` ≠ 没有场外份额**：只有 61% 的 ETF 能反查到联接基金，
  剩下的是债券/商品/部分跨境主题 —— 界面上区分「没有」与「不知道」的成本很高，
  因此文案统一写成「未查到场外联接基金」；
- **不检测退市**：联接基金清盘后不会自动从表中删除（增量刷新只做新增），
  要靠手动全量重建收敛；
