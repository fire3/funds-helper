...
## 13. 净值除权口径（份额分拆 / 分红，2026-09-25 实测）

### 13.1 现象

`159507 通信ETF广发` 在 **2026-06-08 每份基金份额分拆 3.0 份**，于是走势图出现「净值突变」：

| 日期 | 单位净值 | 累计净值 | 上游 `equityReturn` |
|---|---|---|---|
| 2026-06-05 | 3.1334 | 3.1334 | -1.19% |
| 2026-06-08 | 1.0103 | 3.0309 | **-3.27%** |
| 2026-06-09 | 1.0597 | 3.1791 | +4.89% |

单位净值看起来**一天跌了 67.8%**，但上游自己记录的真实涨跌只有 **-3.27%**，
累计净值（3.1334 → 3.0309）也说明当天只跌了 3.27% —— **数据是准的，突变是份额变了**：
拆分后持有份额 ×3、每份净值 ÷3，总市值不变。

### 13.2 真正的 bug：区间统计用了单位净值

修复前 `summarizeNav` 直接拿单位净值做首尾比值，于是 159507 的详情卡显示：

| 区间 | 修复前（单位净值） | 修复后（复权口径） | 上游口径 |
|---|---|---|---|
| 近6月 | **-58.93%** | +23.20% | — |
| 近1年 | **-50.65%** | **+48.05%** | 接口 H `1N` = +48.05% |
| 近3年 | **-2.54%** | **+192.39%** | 接口 H `3N` = +189.56% |
| 近1年最大回撤 | **-76.94%** | **-37.47%** | 接口 A `MAXRETRA1` = 37.47% |

修复后与上游两个独立字段（`1N` 区间涨幅、`MAXRETRA1`）都吻合，
而修复前的近3年 `-2.54%` 与同一张卡片里的「成立来 +166%」自相矛盾。

### 13.3 口径（`packages/core/src/fund/nav.ts`）

1. **走势图仍画原始单位净值**（它本身准确），并在事件当天画虚线标记（`拆分 1:3` / `分红除息`）；
2. **区间涨幅 / 最大回撤改走「可比序列」**：
   - 首选上游 `Data_ACWorthTrend`（累计净值），要求逐点日期对齐且**确实与单位净值不同**
     （相同说明上游没做复权，用了等于没修）；
   - 退化为按拆分比例本地还原（从最后一天往回走，把之后的拆分比例累乘回去，
     分红按「加回每份派现金额」近似）；
   - 没有除权事件时序列与单位净值完全一致（`basis: 'unit'`，不改变任何既有行为）；
3. **事件与解释一起下发**：`navEvents[{ date, kind, detail, ratio, amount, title, text }]`
   + `navSummaryNote`，文案由 core 生成，前端只渲染 —— 避免前后端对「突变」各有一套说法；
4. 上游 `unitMoney` 是唯一线索（`拆分：每份基金份额分拆3.0份` / `每份派现金0.0430元`），
   `sources` 解析时必须保留；认不出的文本按 `kind: 'other'` 保留并提示「当天有净值调整」，
   不静默丢弃。

> 契约兼容：`navEvents` / `navSummaryNote` 在 shared schema 上带 `.default(...)`，
> 详情缓存是持久化的，旧缓存缺字段时降级成「无事件」而不是让前端报结构错误。

### 13.4 测试

| 层 | 用例 |
|---|---|
| `core/fund/nav.test.ts` | 拆分/分红文本解析、累计净值优先、上游没复权时退回本地还原、日期对不齐的兜底、解释文案含比例与真实涨跌 |
| `sources/parsers.test.ts` | `unitMoney` 归一化（空串 → null）、`Data_ACWorthTrend` 二元组解析、fixture `pingzhong/split.js` |
| `server/etf-api.test.ts` | 分拆样本：走势保留台阶、`navEvents` 说明、近1月 = +20%（单位净值口径为 -60%）、回撤 -9.09% |
| `server/qdii-api.test.ts` | 无事件时 `navEvents = []`、`navSummaryNote = null`（不改变既有行为） |
