...
/** 额度文案的视觉处理：渠道不适用用中性色，真实限额越紧越醒目 */
function limitClass(fund: UsdFundRecord): string {
  if (fund.limitText === '渠道不适用') return 'text-slate-400 dark:text-slate-500';
  if (fund.dailyLimit === null) return 'text-slate-500 dark:text-slate-400';
  if (fund.dailyLimit <= 100) return 'font-semibold text-rose-600 dark:text-rose-400';
      ...
      <table className="w-full min-w-[980px] border-collapse text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs text-slate-500 dark:border-slate-800 dark:text-slate-400">
            ...
            <th className="px-3 py-2 font-medium">份额</th>
          ...
          {funds.map((fund) => (
            ...
            >
              ...
              <td className="px-3 py-2">
                <Badge tone={KIND_TONE[fund.usdKind] ?? 'neutral'}>{fund.usdKind}</Badge>
              </td>
              ...
              <td
                className={`tabular px-3 py-2 text-right ${limitClass(fund)}`}
                {...(fund.dailyLimitNote ? { title: fund.dailyLimitNote } : {})}
              >
                {fund.limitText}
              </td>
